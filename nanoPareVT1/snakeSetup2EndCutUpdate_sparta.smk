...
rule setup_sh:
	...
	params:
		...
		runtime_log = config["runtime_log_sparta"]
...
rule endCutSparta_sh:
	...
	params:
		...
		sparta_prog = os.path.join(config["programDir"], config["sPARTADir"], config["5.2.1-spartaprog"]),
		sRna = config["sRna"],
		numShuffledSets = config["numShuffledSets"],
		dataShuffleDir = os.path.join(config["dataDir"], config["shuffleDir"]),
	...
	shell:
		...
		python3 {params.sparta_prog} -genomeFile {params.genomeFasta} -annoType {params.annoType} -annoFile {params.genomeAnnotation} -genomeFeature {params.genomeFeature} -miRNAFile {input.miRfa} -libs <Lib_A.txt Lib_B.txt> -tarPred H -tarScore S --tag2FASTA --map2DD --validate
