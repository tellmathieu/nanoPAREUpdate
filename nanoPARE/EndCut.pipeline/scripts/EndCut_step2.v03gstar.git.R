...
get.detected.sites <- function(dir,sample,suff,window,sRNA.df,shuff.num) {
  ...
  test=data.frame()
  test = read.delim(paste(resultsRoot,'/',folderName,'/',sample,'/anno.mir.tas_',sample,'.',suff,'_win_',window,'_detectedSites.txt',sep=''),sep='\t', row.names=NULL, header=TRUE,stringsAsFactors=FALSE,strip.white=TRUE)
  print(test)
